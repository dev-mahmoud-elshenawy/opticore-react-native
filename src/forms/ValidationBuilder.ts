...
export const email = (message: string = 'Invalid email address') => z.string().email(message);
...
export const password = (options: PasswordValidatorOptions = {}) => {
  ...
  return schema
    ...
    .refine((val: string) => !requireUppercase || /[A-Z]/.test(val), {
      message: 'Must contain an uppercase letter',
    })
    .refine((val: string) => !requireLowercase || /[a-z]/.test(val), {
      message: 'Must contain a lowercase letter',
    })
    .refine((val: string) => !requireNumbers || /\d/.test(val), {
      message: 'Must contain a number',
    })
    .refine((val: string) => !requireSpecial || /[!@#$%^&*(),.?":{}|<>]/.test(val), {
