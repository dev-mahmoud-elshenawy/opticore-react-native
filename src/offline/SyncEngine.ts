...
export class SyncEngine {
    ...
    for (;;) {
      try {
        ...
        if (statusCode === 409) {
          ...
          this.logger.info(
            `[SyncEngine] Conflict detected for ${item.id}, resolving with strategy: ${this.conflictResolver.getStrategy()}`
          );
          ...
          try {
            ...
            if (conflictAttempts > maxRetries) {
              this.logger.error(
                `[SyncEngine] Max conflict retries (${maxRetries}) reached for ${item.id}`,
                error as Error
              );
              throw error;
            }
            this.logger.info(
              `[SyncEngine] Resolved conflict, retrying with new data for ${item.id}`
            );
            // Does NOT consume a network-retry slot or wait for backoff.
            continue;
          } catch (resolveError) {
            this.logger.error(
              `[SyncEngine] Conflict resolution failed for ${item.id}`,
        ...
        if (!this.isRetryable(error)) {
          this.logger.error(`[SyncEngine] Non-retryable error for ${item.id}`, error as Error);
          throw error;
        }
   ...
   */
  private buildReplayHeaders(headers?: Record<string, string>): Record<string, string> | undefined {
