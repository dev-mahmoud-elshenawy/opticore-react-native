...
export class ConsoleTransport implements LogTransport {
  ...
  public write(entry: LogEntry): void {
    ...
    const output = this.formatter ? this.formatter.format(entry) : this.formatDefault(entry);
  ...
  private formatDefault(entry: LogEntry): string {
    ...
    const prefix = useColors ? `${colorPrefix}[${levelLabel}]${reset}` : `[${levelLabel}]`;
    ...
    return showTimestamp === false ? `${prefix} ${message}` : `${prefix} [${timestamp}] ${message}`;
  }
}
