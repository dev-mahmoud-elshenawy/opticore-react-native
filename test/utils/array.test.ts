import { filterNonNull, groupBy, unique, sortBy } from '../../src/utils/array';
...
describe('Array Utilities', () => {
  ...
  describe('sortBy', () => {
    it('sorts array by key ascending', () => {
      ...
      expect(sortBy(input, 'val')).toEqual([{ val: 1 }, { val: 2 }, { val: 3 }]);
    });

    it('sorts array by key descending', () => {
      const input = [{ val: 1 }, { val: 3 }, { val: 2 }];
      expect(sortBy(input, 'val', 'desc')).toEqual([{ val: 3 }, { val: 2 }, { val: 1 }]);
    });
  });
});
