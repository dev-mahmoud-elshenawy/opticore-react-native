...
describe('SyncEngine', () => {
  ...
  beforeEach(() => {
    ...
    mockApiClient = {
      request: jest
        .fn()
        .mockResolvedValue({ data: 'success', status: 200, headers: {}, config: {} }),
    } as any;
  ...
  describe('processQueue', () => {
    it('should process all items successfully', async () => {
      ...
      const result = await engine.processQueue(items, {
        maxRetries: 3,
        retryDelay: 100,
        maxBackoff: 5000,
      });
    ...
    it('should prevent concurrent syncs', async () => {
      ...
      // Start first sync
      const syncPromise = engine.processQueue(items, {
        maxRetries: 3,
        retryDelay: 100,
        maxBackoff: 5000,
      });
  ...
  describe('getSyncingStatus', () => {
    ...
    it('should return true during sync', async () => {
      ...
      mockApiClient.request.mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve({ data: 'success', status: 200, headers: {}, config: {} }), 50)
          )
      );
