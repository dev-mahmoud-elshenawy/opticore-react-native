...
async function runPerformanceTests() {
    ...
    interface LargeItem { id: string; val: number; payload: string; }
    interface LargeState { items: LargeItem[]; addBatch: (items: LargeItem[]) => void; }
    ...
    const largeStore = createBaseStore<LargeState>({
        ...
        addBatch: (newItems: LargeItem[]) => set((state: any) => { state.items.push(...newItems) })
    }) as any);
