...
import { AsyncState, toLoading, toSuccess, toError, isIdle, isSuccess } from '../../src/state/AsyncState';
...
export const useAuthStore = createBaseStore<AuthState>({
    ...
    login: async (name: string) => {
        ...
        try {
            ...
            set((state: any) => {
                state.status = toError(e instanceof Error ? e : new Error('Login failed'), state.status);
            });
        }
    },
...
const productApi = {
    ...
    create: async (data: Partial<Product>) => ({ id: 'p2', name: 'Phone', price: 500, ...data }) as Product
};
...
export function setupGlobalObserver() {
    ...
    // Log errors only
    observer.subscribe(useProductStore, (newState) => {
