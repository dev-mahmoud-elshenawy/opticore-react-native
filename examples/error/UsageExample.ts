...
function handleRenderError(error: unknown) {
    if (error instanceof RenderError) {
        ...
        const strategies = ((error as unknown as Record<string, unknown>).strategies || []) as RecoveryStrategy[];
        if (strategies.length > 0) {
            console.log('[UI] Offering Recovery:', strategies.map((s: RecoveryStrategy) => s.label));
        }
    } else {
